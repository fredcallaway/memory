...
The optimal model predicts a weak positive effect, while humans almost show a
negative effect.

*NEW*: Previously, the model predicted a stronger effect here. The difference
...
the first fixation). I'm not sure if this is a reasonable thing to do.
Essentially, we are qualitatively "fitting" the evidence accumulation rate to 
...
Previously, we had a significant effect in the human data. But excluding the
...
we aren't seeing anything in either random model, I think this result might be solid.
...
The effect of relative memory strength on fixation proportion mostly disappears
when we control for the effect of the last fixation. In the optimal model (with
these parameters), it disappears completely!

However, we still need to explain why we see the overall proportion effect
in both the human data and optimal simulations, but *not* either of the
random models. It seems likely that something rational is at play here.

It turns out, the overall proportion effect is primarily driven (we think) by
the interaction between two other effects:
...
This effect occurs mechanistically because
(1) the currently fixated cue is almost always the one remembered,
(2) remembering the cue terminates the trial, making this fixation the final one, and
...
this fixation is longer than all the others. It immediately follows that
