...
multi %>% 
    group_by(wid) %>% 
    left_join(
        afc_scores %>% summarise(med_str = median(strength))
    ) %>% 
    mutate(
        first_strong = strength_first > med_str,
        second_strong = strength_second > med_str,
        test = first_strong + second_strong
    ) %>% select(test)
```
...
multi %>% 
    filter(wid == "w2f1698c") %>% 
    select(trial_num, strength_first, strength_second)  %>% 
    mutate(
        strength_high = pmax(strength_first, strength_second),
        strength_low = pmin(strength_first, strength_second),
    ) %>% 
    pivot_longer(c(strength_high, strength_low), names_to="cue", names_prefix="strength_", values_to="strength") %>%
    ggplot(aes(trial_num, strength, color=cue)) + geom_point()
