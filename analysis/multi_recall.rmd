...
multi = multi %>% 
    ...
    mutate(
        relative_strength = strength_first - strength_second,
...
lmer(choose_first ~ relative_strength + (relative_strength|wid), data=multi) %>% summ
...
X %>% lmer(prop_first ~ relative_strength + (relative_strength|wid), data=.) %>% summ
...
more rational than others. This could be a direction to explore more.
...
X %>% ggplot(aes(relative_strength, second_pres_time)) + 
    stat_summary_bin(fun.data=mean_cl_boot, bins=10) +
    geom_smooth(method='lm')

X %>% lmer(second_pres_time ~ relative_strength + (relative_strength|wid), data=.) %>% summ
```
