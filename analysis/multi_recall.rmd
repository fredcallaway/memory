...
multi = multi %>% 
    ...
    mutate(
        first_advantage = strength_z_first - strength_z_second,
        chosen_strength = if_else(choose_first, strength_z_first, strength_z_second),
    )
...
```{r}
ggplot(multi, aes(strength_z_first, as.numeric(choose_first))) + 
...
We can also look at the difference between first and second. It's a worse
predictor, but both do contribute (when specified as separate terms).

```{r}
ggplot(multi, aes(strength_z_first - strength_z_second, as.numeric(choose_first))) + 
    stat_summary_bin(fun.data=mean_cl_boot, bins=5) +
    geom_smooth(method = "glm", method.args = list(family = "binomial"), formula=y~x)

lmer(choose_first ~ I(strength_z_first - strength_z_second) + (1|wid), data=multi) %>% summ
lmer(choose_first ~ strength_z_first + strength_z_second + (1|wid), data=multi) %>% summ
```
...
multi %>% 
    ggplot(aes(strength_z_first + strength_z_second, choice_rt)) + 
    stat_summary_bin(fun.data=mean_cl_boot, bins=5) + 
    geom_smooth()
...
multi %>% 
    filter(n_pres == 1) %>%
    ggplot(aes(strength_z_first, choice_rt)) +
    stat_summary_bin(fun.data=mean_cl_boot, bins=5) +
    geom_smooth(method=lm)

multi %>% 
    ggplot(aes(strength_z_first, as.numeric(n_pres == 1))) +
    stat_summary_bin(fun.data=mean_cl_boot, bins=5) +
    geom_smooth(method = "glm", method.args = list(family = "binomial"), formula=y~x)
```
