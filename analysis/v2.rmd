---
title: Predicting recall RT with 2AFC RT (v2.0B)
date: March 4, 2021
---
...
## Simulating pairings
For example, x axis are index for 10 pairs ordered by (easy-hard), there is a red line representing easy and blue line representing hard. each data point on red or blue is mean of 30 subjects, then add error bar - then we can see if the blue and red line get more apart as the (easy-hard) is increasing
...
X = afc %>%
    ...
    left_join(select(trials, wid, word, rt))

ggplot(X, aes(pair, rt, color=kind)) +
    stat_summary() +
    geom_smooth(method='lm', se=F)

d = X %>% 
    pivot_wider(c(wid, pair), values_from=rt, names_from=kind) %>%
    mutate(diff = hard - easy)

ggplot(d, aes(pair, diff)) + 
    stat_summary() + 
    geom_smooth(method='lm', se=F) + 
    geom_hline(yintercept=0)

d %>% with(lm(diff ~ pair)) %>% summ(scale=TRUE)
```
