...
human = human %>% mutate(
    ...
    primed = factor(primed, levels=c("second", "first"))
)
...
prime_effect = function(data, yvar) {
    model = data %>% tidymer(primed, {{yvar}})
    model %>% summ %>% smart_print
    
    model %>% ggpredict(terms="primed") %>% 
...
human %>% 
    ...
    prime_effect(choose_first) + ylim(0,1)
```
...
long %>% 
    ...
    ggplot(aes(presentation, duration, color=as_factor(first_primed))) +
