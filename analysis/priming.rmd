...
```{r third-interact1, fig.width=4, fig.height=HEIGHT}
human %>% 
    filter(n_pres >= 4) %>% 
    mutate(first_primed = int(first_primed)) %>% 
...
For completeness, here are all the main memory strength effects from
Experiment 1, adding an interaction with the prime. The simple ones look very
reasonable. The fixation durations are all really noisy, so I'm not going to
interpret them.
