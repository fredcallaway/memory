...
read_sim = function(name) {
    read_csv(glue("../model/results/sim_{name}_apr20.csv")) %>% mutate(name=!!name)
}
...
df %>% 
    filter(n_pres >= 2) %>% 
    ggplot(aes(rel_strength, prop_fix)) + geom_smooth(se=F) + facet_wrap(~name)
...
```{r,fig.width=7.5, fig.height=3}
ggplot(df, aes(strength_first, first_fix_duration)) + geom_smooth(se=F) + facet_wrap(~name)
```
