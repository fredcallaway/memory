...
afc_scores = afc %>% 
    filter(block > 1) %>% 
    mutate(score =  5 * correct - log(rt)) %>%
    group_by(wid, word) %>% 
    summarise(score = mean(score)) %>%
    group_by(wid) %>%
    mutate(strength = scale(raw_strength))zz

all_trials = left_join(all_trials, afc_scores)
...
trials = trials %>% filter(rt > 50) %>% filter(n() >= 35)
...
trials %>% 
    filter(response_type == "empty") %>%
    with(lmer(rt ~ strength + (1|wid))) %>% summ
```

```{r}
ggplot(trials, aes(afc_rt/1000, recall_rt/1000, group=wid)) + 
    geom_point(alpha=0.2) +
    geom_smooth(method=lm, se=F) +
    xlim(0, 5) + ylim(0, 5) + 
    labs(x="2AFC RT (s)", y="Cued Recall RT (s)")
```
...
lmer(log_recall_rt ~ strength + (1|wid), data=trials) %>% summ

X = trials %>% 
    select(log_recall_rt, strength, wid) %>% 
    drop_na()

m = gam(log_recall_rt ~ s(strength, bs = "cs", k = 10) +
        s(wid, strength, bs='re'), data=X, method="REML")

m = gam(log_recall_rt ~ s(strength, bs = "cs", k = 10) +
                        s(wid, strength, bs='re'),
        data=X, method="REML")
...
preds %>% ggplot(aes(strength, log_recall_rt, group=wid)) + 
    geom_line(size=.5) + theme_classic()
...
X = afc %>%
    ...
    summarise(score=mean(score)) %>% 
    group_by(wid) %>% 
    right_join(select(trials, wid, word, recall_rt=rt)) %>% 
    drop_na

stopifnot(nrow(X) == nrow(trials))

X %>% ggplot(aes(score, log(recall_rt), group=wid)) + 
    geom_point(alpha=0.2) +
    geom_smooth(method=lm, se=F) + 
    labs(x="2AFC Log RT", y="Cued Recall Log RT")
```
