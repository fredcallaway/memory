...
multi = load_data('multi-recall') %>%
    ...
    ungroup() %>% 
    # filter(trial_num > 5) %>% # DROP FIRST HALF OF TRIALS
    mutate(
...
First the main results: predicting behavior in the multi-cue trials from 
...
X2 %>% ggplot(aes(rel_strength, prop_first)) +
    stat_summary_bin(fun.data=mean_cl_boot, bins=10) +
    geom_smooth(method='lm')
...
## Proportion split by number of presentations
People remember the thing they look at last `r round(100 * mean(multi$choose_last_seen, na.rm=T))`% of the time.
Could that be driving the effect on proportion? One way to answer this is to split by number
of presentation. Ideally we'll see the same positive relationship in each group.

```{r}
multi %>% 
    filter(n_pres >= 2) %>%
    ggplot(aes(rel_strength, prop_first, color=odd_pres)) +
        stat_summary_bin(fun.data=mean_cl_boot, bins=10) +
        geom_smooth(method='lm')

multi %>% 
    filter(n_pres > 1) %>%
    lmer(prop_first ~ rel_strength * odd_pres + (1 | wid), data=.) %>% summ
```

We don't see it. However, it's worth noting that the number of presentations depends on 
the strength. The same pattern holds in the Optimal and Random model (in the latter
it's maybe weaker but could just be a parameter thing). 

TODO figure out if the random model can produce this pattern.
...
X %>% ggplot(aes(strength_first, fpt_z)) + 
    stat_summary_bin(fun.data=mean_cl_boot, bins=10) +
    geom_smooth(method='lm')
...
Y = X %>%
    group_by(wid) %>% 
    mutate(
        first_pres_time = scale(first_pres_time),
        strength_first = scale(strength_first)
    ) 

Y %>% lmer(first_pres_time ~ strength_first + (strength_first|wid), data=.) %>% summ


Y %>% 
    ggplot(aes(strength_first, first_pres_time, group=wid)) + 
    # stat_summary_bin(fun.data=mean_cl_boot, bins=10) +
    geom_smooth(method='lm', se=F) +
    geom_smooth(method='lm', se=F, group=0, color="black")
```
...
X = multi %>% 
    ...
    filter(n_pres > 2) %>% 
    mutate(spt_z = scale(second_pres_time))

X %>% ggplot(aes(strength_second, spt_z)) + 
    stat_summary_bin(fun.data=mean_cl_boot, bins=10) +
    geom_smooth(method='lm')
...
```{r}
X %>% 
    mutate(strength_split = if_else(strength_first > 0, "high strength", "low strength")) %>% 
    ggplot(aes(first_pres_time)) + 
    geom_histogram() 
    facet_wrap(~strength_split)
```
...
```{r}
X = simple %>% 
    mutate(presentation = (trial_num-1) %/% 20 + 1) %>% 
...
X %>% ggplot(aes(rt_z2, rt_z3)) + geom_point() + geom_smooth() + 
    labs(x="second presentation RT (zscored)", y="third presentation RT (zscored)")

with(X, cor(X %>% select(3:5), use='comp'))
```
...
X = simple %>% 
    mutate(presentation = (trial_num-1) %/% 20 + 1) %>% 
    select(wid, word, correct, presentation) %>% 
    pivot_wider(names_from=presentation, values_from=correct, names_prefix="correct") %>% 
    ungroup()

X %>% ggplot(aes(correct2, as.numeric(correct3))) + stat_summary(fun.data=mean_cl_boot) + 
    labs(x="second presentation correct", y="third presentation accuracy")
...
both = inner_join(rt, choice) 

both %>% ggplot(aes(rt_z2, as.numeric(correct3), color=correct2)) + 
    stat_summary_bin(fun.data=mean_cl_boot, bins=5) +
    geom_smooth() +
    labs(x="second presentation correct", y="third presentation accuracy")
    
both %>% ggplot(aes(rt_z2, rt_z3, color=correct2)) + geom_point() + geom_smooth() +
    labs(x="second presentation RT (zscored)", y="third presentation RT (zscored)")
# both %>% ggplot(aes(rt_z2, rt_z3, color=interaction(correct2, correct3))) +
#     geom_smooth() +
#     labs(x="second presentation RT (zscored)", y="third presentation RT (zscored)"

```

This last one is potentially worth thinking more about. Does this suggest
that we should be using an interaction between accuracy and RT to measure
memory strength?