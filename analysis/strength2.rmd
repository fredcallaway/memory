...
```{r}
model = read_csv("../model/results/sim_gaussian2.csv")
...
For each fixation number, we have relative (fixated vs. non-fixated), fixated, non-fixated
except the first which only has fixated.

```{r, fig.width=WIDTH, fig.height=HEIGHT}
p1 = df %>% #plot
    filter(n_pres >= 2) %>% 
    regress(pre_correct_first, first_pres_time, bins=0, bin_range=1) +
    stat_summary(fun.data=mean_cl_boot, size=.2)

p2 = df %>% 
    filter(n_pres >= 3) %>% 
    mutate(rel_pre_correct = -rel_pre_correct) %>% 
...
p5 = df %>% 
    filter(n_pres >= 4) %>% 
    filter(pre_correct_first != 0.5) %>% 
    filter(pre_correct_second != 0.5) %>% 
...
p1 = df %>% #plot
    filter(n_pres >= 2) %>% 
    regress(pre_correct_z_first, first_pres_time)

p2 = df %>% 
    filter(n_pres >= 3) %>% 
    mutate(rel_pre_correct_z = -rel_pre_correct_z) %>% 
    regress(rel_pre_correct_z, second_pres_time) # plot

p3 = df %>% 
    filter(n_pres >= 3) %>% 
    regress(pre_correct_z_second, second_pres_time) # plot

p4 = df %>% 
    filter(n_pres >= 3) %>% 
    regress(pre_correct_z_first, second_pres_time) # plot

p5 = df %>% 
    filter(n_pres >= 4) %>% 
    filter(pre_correct_z_first != 0.5) %>% 
    filter(pre_correct_z_second != 0.5) %>% 
    regress(rel_pre_correct_z, third_pres_time) # plot

p6 = df %>% 
    filter(n_pres >= 4) %>% 
    regress(pre_correct_z_first, third_pres_time) # plot

p7 = df %>% 
    filter(n_pres >= 4) %>% 
    regress(pre_correct_z_second, third_pres_time) # plot
...
p2 = df %>% 
    filter(n_pres >= 3) %>% 
    mutate(rel_pre_logrt_z = -rel_pre_logrt_z) %>% 
...
```{r}
df %>% 
    mutate(is_final=1*(n_pres == 1)) %>% 
    regress_interaction(pre_logrt_z_first, is_final, first_pres_time)

df %>% 
    mutate(is_final=1*(n_pres == 2)) %>% 
    regress_interaction(pre_logrt_z_second, is_final, second_pres_time)

df %>% 
    mutate(is_final=1*(n_pres == 3)) %>% 
    regress_interaction(pre_logrt_z_first, is_final, third_pres_time)

df %>% 
    filter(n_pres > 1) %>% 
    regress(pre_logrt_z_first, first_pres_time)

df %>% 
    filter(n_pres > 2) %>% 
    regress(pre_logrt_z_second, second_pres_time)

df %>% 
    filter(n_pres > 3) %>% 
    regress(pre_logrt_z_first, third_pres_time)


df %>% 
    mutate(is_final=n_pres == 2) %>% 
    ggplot(aes(pre_logrt_z_second, second_pres_time, color=is_final)) +
...
groups = df %>% nest_by(wid, .keep=TRUE) %>% with(data)
...
pwr1 = power_analysis(N, n_sim, . %>% 
    filter(n_pres >= 2) %>% 
    lmer(first_pres_time ~ pre_correct_first + (pre_correct_first|wid), data=.) %>% 
    summ %>% with(coeftable[2, "p"])
)
```

```{r}
pwr1 %>% ggplot(aes(p, alpha=0.1)) +
...
pwr1 %>% 
    ...
    ggplot(aes(N, power)) + geom_line() + ylim(0.5, 1)
```
