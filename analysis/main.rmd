...
- In the human data, error trials (which the model never predicts) are excluded.
- In the model, strength is now proportional to $\log(p)$---previously it was just $p$.
  The motivation for this is that reaction time in a single cue recall task 
  is proportional to $1 / p$, which implies $\log(p) = C - \log(t)$ for some
...
- We also add Gaussian noise to the model strength to account for the fact
  that the human strength measure is very noisy. Ideally we would have a more
  principled way to do this (based on a generative model).
...
Z-score $\log(p)$ to put it in roughly the same units. The motivation for this
is that reaction time in a single cue recall task is proportional to $1 / p$,
which implies $\log(p) = C - \log(t)$ for some unknown constant $C$. Because
our human strength measure is in units of $\log(t)$, it makes sense for the
model strength to be proportional $\log(p)$.
...
Previously the model made a strong prediction here, but with the new
parameters, this is no longer the case.
...
I previously thought this could be due to selection effects, but I'm not exactly sure
what those would be. I think we can interpret this result as evidence as long as we 
get a null result in a convincing random model.
...
The optimal model prediction is a lot weaker than before because noise is added to the strength.

We no longer see a spike at the end in the Random model. I think this must be due to the 
commitment mechanism, but I have a hard time reconciling the null effect here with the 
substantial effect of cue strength on cue selection probability ([Probability of remembering first word])
