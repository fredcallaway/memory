...
::: {.alert .alert-info}
Unlike the last "main" report, we are _not_ excluding the first round
of 2AFC trials when computing memory strength.
:::
...
### Number and finality
```{r, fix-by-number-finality, fig.width=7.5, fig.height=3.3}
long %>% 
...
```{r,fig.width=7.5, fig.height=3}
long %>%
    ggplot(aes(duration/1000, color=factor(last_fix))) +
    xlim(0, 5) +
    facet_wrap(~name) +
    geom_density()

long %>%
    ggplot(aes(duration/1000)) +
    xlim(0, 5) +
    facet_wrap(~name) +
    geom_density()
...
df %>% 
    ...
    stat_summary_bin(fun.data=mean_cl_boot, bins=5) + 
    facet_wrap(~name) +
    labs(x="Strength of Selected Cue", y="Reaction Time")
...
X %>% filter(name == "Human") %>% lmer(last_pres_time ~ last_strength + (last_strength|wid), data=.) %>% summ
```
...
X %>% filter(name == "Human") %>% lmer(first_pres_time ~ strength_first + (strength_first|wid), data=.) %>% summ
...
X %>% 
    ggplot(aes(strength_second, second_pres_time)) +
    geom_smooth() +
    stat_summary_bin(fun.data=mean_cl_boot, bins=5) + 
    facet_wrap(~name)

X %>% filter(name == "Human") %>% lmer(second_pres_time ~ strength_second + (strength_second|wid), data=.) %>% summ
